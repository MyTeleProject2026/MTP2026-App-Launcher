...
function App() {
  ...
  return <div className={`app-shell ${settings.compactMode ? 'compact-mode' : ''}`}>
    ...
    <aside className={`sidebar ${sidebar ? 'open' : ''}`}>
      ...
      {menu && <div className="account-menu"><div className="account-head"><VexaAvatar profile={profile}/><div><b>{name}</b><small>{profile?.email || 'VexaAccount'}</small></div></div><hr/>{logged ? <button onClick={() => doLogout()}><LogOut/> Sign out</button> : <button onClick={login}><LogIn/> Sign in with VexaAccount</button>}<button onClick={() => { setShowProfile(true); setMenu(false); }}><UserRound/> VexaAccount profile</button><button onClick={() => window.open('https://vexaaccount-management.onrender.com','_blank','noopener,noreferrer')}><Settings/> Manage VexaAccount</button><button onClick={() => { setShowSettings(true); setMenu(false); }}><Settings/> Launcher Settings</button><button onClick={switchAccount}><ArrowRightLeft/> Switch account</button></div>}
    </aside>
