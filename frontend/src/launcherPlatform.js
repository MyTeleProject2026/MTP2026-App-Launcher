...
export function getDeviceMode(mode) {
  return DEVICE_MODES[mode] ? mode : 'android';
}

export function DeviceModeSettings({ value, onChange }) {
  const current = getDeviceMode(value);
  return <div className="device-mode-grid">
    {Object.entries(DEVICE_MODES).map(([id, item]) => {
      const Icon = item.icon;
      return <button key={id} type="button" className={`device-mode-card ${current === id ? 'active' : ''}`} onClick={() => onChange(id)}>
...
export function ApplicationSettingsModal({ app, onClose, onPatch, onRemove, onOpen }) {
  ...
  async function upload(file) {
    ...
    try {
      ...
      await onPatch(app.id, 'userIconUrl', value);
      setIcon(value);
