...
const issuer = (import.meta.env.VITE_VEXA_ACCOUNT_ISSUER_URL || '').replace(/\/$/, '');
...
export async function startVexaLogin() {
  ...
  window.location.assign(`${issuer}/api/sso/authorize?${params}`);
}
...
export async function finishVexaLogin() {
  ...
  if (!code) return null;
  const expectedState = sessionStorage.getItem('mtp_oauth_state');
  const verifier = sessionStorage.getItem('mtp_pkce_verifier');
  if (!state || !expectedState || state !== expectedState || !verifier) throw new Error('OAUTH_STATE_MISMATCH');
  ...
  if (!response.ok) throw new Error(data.error || 'SSO_LOGIN_FAILED');
  sessionStorage.removeItem('mtp_pkce_verifier');
  sessionStorage.removeItem('mtp_oauth_state');
  localStorage.setItem('mtp_access_token', data.access_token);
  localStorage.setItem('mtp_profile', JSON.stringify(data.profile || {}));
