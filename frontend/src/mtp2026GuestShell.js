...
function render() {
  ...
  const appButtons = apps.map(app => `<button data-app-url="${esc(app.url)}"><img src="${esc(app.iconUrl || '/branding/mtp2026-mark.svg')}" alt=""><span>${esc(app.name || 'VexaApp')}</span></button>`).join('');

  if (profile.layout === 'desktop') {
    root.innerHTML = `<div class="mtp-guest-desktop"><header><img src="/branding/mtp2026-mark.svg" alt="MTP2026"><strong>${esc(profile.label)}</strong><span class="mtp-guest-clock">${new Date().toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}</span></header><main><aside><button data-action="start">▦ Start</button><button data-action="store">VexaStore</button><button data-action="settings">Settings</button><button data-action="files">Files</button></aside><section><div class="mtp-guest-desktop-welcome"><span>${esc(profile.shortLabel)}</span><h2>${esc(profile.label)}</h2><p>ARM64 guest workspace · VexaAccount session · VexaStore applications</p></div><div class="mtp-guest-apps">${appButtons || '<small>No VexaStore apps installed yet.</small>'}</div></section></main><footer><span>⌁ ${esc(profile.family)}</span><span>ARM64</span><span>VexaAccount</span></footer></div>`;
  ...
  root.querySelectorAll('[data-app-url]').forEach(b => b.onclick = () => openUrl(b.dataset.appUrl));
