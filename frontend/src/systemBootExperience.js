(() => {
  ...
  const labels = {android:'MTP2026 Android OS',ios:'MTP2026 Device OS',windows:'MTP2026 Desktop OS',windows11:'MTP2026 Desktop OS',gaming:'MTP2026 Gaming OS'};
  let splash = null;
  let timer = null;
  ...
  window.addEventListener('mtp2026:guest-image-required',event=>{
    ...
    if (event.detail?.provider === 'web-os-shell') { remove(); return; }
    showRecovery(event,event.detail?.code);
  });
  window.addEventListener('mtp2026:guest-boot-error',event=>{
    if (!window.MTP2026NativeGuestRuntime?.bootGuest) { remove(); return; }
    if (String(event.detail?.error || '').includes('web-os-shell')) { remove(); return; }
