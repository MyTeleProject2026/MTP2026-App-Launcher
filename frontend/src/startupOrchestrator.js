...
async function getSession() {
  ...
  try {
    const response = await fetch(`${SESSION_API}/auth/session`, { credentials: 'include', headers: { Accept: 'application/json' }, cache: 'no-store', signal: controller.signal });
...
function persistState(session, arm64) {
  localStorage.setItem(SESSION_KEY, JSON.stringify({ checkedAt: Date.now(), authenticated: Boolean(session?.profile), arm64: arm64?.state || 'unknown', virtualArm64: arm64?.virtualArm64 !== false }));
}
...
async function run() {
  ...
  const hardStop = window.setTimeout(() => {
    ...
    hideOverlay();
  }, MAX_STARTUP_BLOCK_MS);

  try {
    const session = await getSession();
    if (session?.__error) {
      setText('MTP2026 is ready', 'The session service did not answer in time. The normal VexaAccount sign-in screen remains available and the launcher will stay interactive.');
      step('session', 'blocked', 'Service timeout');
      step('arm64', '', 'Available after sign-in');
      step('system', '', 'Available after sign-in');
    ...
    if (!session?.profile) {
      ...
      setText('Sign in to MTP2026', 'Use VexaAccount SSO first. The launcher remains interactive while authentication is completed.');
      ...
      window.setTimeout(() => { window.clearInterval(poll); hideOverlay(); }, MAX_STARTUP_BLOCK_MS);
      return;
    }
    ...
    setText('Choose your MTP2026 system', physical ? 'ARM64 boot target is ready. Choose the system mode to open.' : 'Choose the launcher system mode.');
