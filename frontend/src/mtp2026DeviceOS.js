...
const SYSTEM_APPS = [
  ...
  { id: 'files', name: 'Files', glyph: '▤', subtitle: 'MTP storage', kind: 'files' },
  { id: 'browser', name: 'Browser', glyph: '◎', subtitle: 'Web browsing', kind: 'browser' },
...
function isIOSMode() {
  return (document.documentElement.dataset.mtpDeviceMode || localStorage.getItem('mtp2026-default-system-os')) === MODE;
}
...
function logicalPartitions(storage) {
  ...
  return [
    { id: 'system', name: 'System', icon: '', size: Math.round(quota * .08), desc: 'MTP2026 system shell and configuration' },
    { id: 'user', name: 'User', icon: '⌂', size: Math.round(quota * .45), desc: 'User documents and imported files' },
    { id: 'apps', name: 'Applications', icon: '▦', size: Math.round(quota * .35), desc: 'Installed web applications and metadata' },
...
async function loadSessionAndApps() {
  ...
  try {
    ...
    if (response.ok) state.apps = await response.json();
  } catch (_) { state.apps = []; }
}

function profileName() {
  return state.session?.profile?.name || state.session?.profile?.email || 'VexaAccount User';
}

function appItems() {
  const external = state.apps.map(app => ({
    id: `external-${app.id}`,
    name: app.title || 'Application',
    glyph: '●',
    url: app.url,
    subtitle: app.pwaSupported ? 'Installed web app' : 'Web app',
    kind: 'external',
    source: app,
  }));
...
function openUrl(url) {
  try { window.MTP2026NativePlatform?.nativeOpenExternal?.(url); } catch (_) { window.open(url, '_blank', 'noopener,noreferrer'); }
}

function hideRoot(hidden) {
...
function appIcon(item) {
  if (item.source?.userIconUrl || item.source?.iconUrl) return `<img src="${esc(item.source.userIconUrl || item.source.iconUrl)}" alt=""/>`;
  return `<span>${esc(item.glyph)}</span>`;
}

function renderHome() {
  const root = ensureRoot();
  const items = appItems().filter(item => {
    const q = state.search.trim().toLowerCase();
    return !q || `${item.name} ${item.subtitle}`.toLowerCase().includes(q);
  });
  root.innerHTML = `
    <div class="mtp-ios-shell">
      ...
      <div class="mtp-ios-home">
        <header class="mtp-ios-header"><div><span class="mtp-ios-eyebrow">MTP2026 DEVICE OS</span><h1>MTP2026</h1><p>${esc(profileName())}</p></div><button class="mtp-ios-avatar" data-action="account">${esc((profileName()[0] || 'V').toUpperCase())}</button></header>
      ...
      <div class="mtp-ios-home-indicator"></div>
    </div>`;
...
function openModal(title, body) {
  const root = ensureRoot();
  let modal = root.querySelector('.mtp-modal');
  if (!modal) { modal = document.createElement('div'); modal.className = 'mtp-modal'; root.appendChild(modal); }
  modal.innerHTML = `<div class="mtp-modal-card"><header><b>${esc(title)}</b><button data-close>×</button></header><section>${body}</section></div>`;
  modal.querySelectorAll('[data-close]').forEach(b => b.onclick = () => modal.remove());
}

function openApp(id) {
  if (id === 'home') { state.page = 'home'; render(); return; }
  const item = appItems().find(x => x.id === id);
  if (!item) return;
  state.page = item.kind;
  if (item.kind === 'settings') return renderSettings();
  if (item.kind === 'files') return renderFiles();
  if (item.kind === 'store' || item.kind === 'account' || item.kind === 'browser' || item.kind === 'external') return openUrl(item.url);
  if (item.kind === 'coming') return openModal(item.name, `<p>${esc(item.name)} is registered in the MTP2026 Device OS shell and will become available when its official VexaApp web service is published.</p><button data-close>Close</button>`);
}

async function render() {
  if (!isIOSMode()) { document.getElementById(ROOT_ID)?.remove(); hideRoot(false); return; }
  hideRoot(true); await loadSessionAndApps(); await refreshStorage();
  if (state.page === 'settings') return renderSettings();
  if (state.page === 'files') return renderFiles();
  renderHome();
}

function syncMode() { if (isIOSMode()) { if (!document.getElementById(ROOT_ID)) render(); } else { document.getElementById(ROOT_ID)?.remove(); hideRoot(false); } }

window.addEventListener('mtp2026:default-system-os', syncMode);
window.addEventListener('storage', e => { if (e.key === 'mtp2026-default-system-os') syncMode(); });
const modeObserver = new MutationObserver(() => syncMode());
modeObserver.observe(document.documentElement, { attributes: true, attributeFilter: ['data-mtpDeviceMode'] });
window.addEventListener('load', () => setTimeout(syncMode, 80));
