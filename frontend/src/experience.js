(() => {
  ...
  const api = async (path, options = {}) => {
    ...
    for (let attempt = 0; attempt <= RETRY_DELAYS_MS.length; attempt += 1) {
      ...
      try {
        const response = await fetch(`${API}${path}`, { credentials: 'include', cache: 'no-store', signal: controller.signal, ...options, headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', ...(options.headers || {}) } });
        ...
        const retryable = /FETCH_FAILED|REQUEST_TIMEOUT|Request failed \((502|503|504)\)/.test(String(lastError.message || ''));
  ...
  async function loadState({ force = false } = {}) {
    ...
    loadPromise = (async () => {
      try {
        ...
        console.warn('[MTP2026] experience state load failed:', error);
