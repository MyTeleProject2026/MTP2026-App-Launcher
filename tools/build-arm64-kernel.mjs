...
execFileSync('rustup', ['target', 'add', target], { cwd: root, env, stdio: 'inherit' });
execFileSync('cargo', ['build', '--release', '--target', target, '--manifest-path', manifest], { cwd: root, env, stdio: 'inherit' });
if (!existsSync(elf)) throw new Error(`ARM64 kernel ELF was not produced: ${elf}`);

const sysroot = execFileSync('rustc', ['--print', 'sysroot'], { cwd: root, env, encoding: 'utf8' }).trim();
