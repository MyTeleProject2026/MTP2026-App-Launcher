...
cp -a "$RUNTIME_DIR/chromium" "$OUT/usr/lib/chromium"
cp -a "$RUNTIME_DIR/chromium-share" "$OUT/usr/share/chromium"
cp -a "$RUNTIME_DIR/aarch64-linux-gnu/." "$OUT/usr/lib/"
cp -a "$RUNTIME_DIR/lib-aarch64-linux-gnu/." "$OUT/lib/"
cp -a "$RUNTIME_DIR/ld-linux-aarch64.so.1" "$OUT/lib/ld-linux-aarch64.so.1"
if [ -f "$RUNTIME_DIR/icudtl.dat" ]; then cp -a "$RUNTIME_DIR/icudtl.dat" "$OUT/usr/lib/chromium/icudtl.dat"; fi
cp -a "$RUNTIME_DIR/certs" "$OUT/etc/ssl/certs"
cp -a "$RUNTIME_DIR/fonts/." "$OUT/etc/fonts/"
chmod +x "$OUT/usr/bin/chromium" "$OUT/usr/lib/chromium/chromium"
cp -a "$RUNTIME_DIR/MANIFEST" "$OUT/MANIFEST"
touch "$MARKER"
