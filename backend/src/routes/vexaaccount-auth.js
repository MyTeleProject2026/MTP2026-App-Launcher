...
export function registerVexaAuthRoutes(app,{pool,ensureUser}) {
  ...
  app.post('/api/auth/logout',async(req,res)=>{const id=readCookies(req)[SESSION_COOKIE];if(id&&pool){try{const [rows]=await pool.execute('SELECT refresh_token_enc FROM mtp_sso_sessions WHERE id=? LIMIT 1',[id]);if(rows[0]?.refresh_token_enc)await revokeVexaSession(decryptSecret(rows[0].refresh_token_enc)).catch(()=>false);}catch{}await pool.execute('DELETE FROM mtp_sso_sessions WHERE id=?',[id]).catch(()=>{});}res.setHeader('Set-Cookie',serializeCookie(SESSION_COOKIE,'', {...SESSION_COOKIE_OPTIONS,maxAge:0}));res.status(204).end();});
  ...
  app.get('/api/settings',auth,async(req,res)=>{try{await authTablesReady;const [rows]=await pool.execute('SELECT theme,default_view AS defaultView,open_behavior AS openBehavior,compact_mode AS compactMode,device_mode AS deviceMode FROM mtp_user_preferences WHERE user_id=?',[req.mtpSession.userId]);res.json(rows[0]||{theme:'system',defaultView:'launcher',openBehavior:'new_tab',compactMode:false,deviceMode:'android'});}catch{res.status(500).json({error:'SETTINGS_LOAD_FAILED'});}});
  app.patch('/api/settings',auth,async(req,res)=>{try{await authTablesReady;const body=req.body||{};const allowed={theme:['system','light','dark'],defaultView:['launcher','applications','favorites','recent'],openBehavior:['new_tab','same_tab'],compactMode:[true,false],deviceMode:['mtp2026','android','desktop','gaming']};for(const key of Object.keys(body)){if(!allowed[key]||!allowed[key].includes(body[key]))return res.status(400).json({error:'INVALID_SETTING'});}const userId=req.mtpSession.userId;await pool.execute('INSERT INTO mtp_user_preferences(user_id) VALUES(?) ON DUPLICATE KEY UPDATE user_id=user_id',[userId]);if(Object.hasOwn(body,'theme'))await pool.execute('UPDATE mtp_user_preferences SET theme=? WHERE user_id=?',[body.theme,userId]);if(Object.hasOwn(body,'defaultView'))await pool.execute('UPDATE mtp_user_preferences SET default_view=? WHERE user_id=?',[body.defaultView,userId]);if(Object.hasOwn(body,'openBehavior'))await pool.execute('UPDATE mtp_user_preferences SET open_behavior=? WHERE user_id=?',[body.openBehavior,userId]);if(Object.hasOwn(body,'compactMode'))await pool.execute('UPDATE mtp_user_preferences SET compact_mode=? WHERE user_id=?',[body.compactMode?1:0,userId]);if(Object.hasOwn(body,'deviceMode'))await pool.execute('UPDATE mtp_user_preferences SET device_mode=? WHERE user_id=?',[body.deviceMode,userId]);res.json({ok:true});}catch{res.status(500).json({error:'SETTINGS_UPDATE_FAILED'});}});

  app.get('/api/notifications',auth,async(req,res)=>{try{const parsedLimit=Number(req.query.limit);const limit=Number.isFinite(parsedLimit)?Math.min(100,Math.max(1,Math.trunc(parsedLimit))):30;const [rows]=await pool.execute(`SELECT id,type,title,message,read_at AS readAt,created_at AS createdAt FROM mtp_notifications WHERE user_id=? ORDER BY created_at DESC LIMIT ${limit}`,[req.mtpSession.userId]);res.json(rows);}catch(error){console.error('MTP2026 notifications load failed',error?.message||error);res.status(500).json({error:'NOTIFICATIONS_LOAD_FAILED'});}});
  ...
  app.get('/api/apps/experience',auth,async(req,res)=>{try{await authTablesReady;const [rows]=await pool.execute(`SELECT ua.application_id AS id,ua.category,ua.is_favorite AS favorite,ua.is_pinned AS pinned,ua.custom_icon_data AS customIconData,a.title,a.canonical_url AS url,a.icon_url AS iconUrl,a.pwa_supported AS pwaSupported FROM user_applications ua JOIN applications a ON a.id=ua.application_id WHERE ua.user_id=? ORDER BY ua.is_pinned DESC,ua.is_favorite DESC,ua.sort_order,a.title`,[req.mtpSession.userId]);res.json(rows);}catch{res.status(500).json({error:'APPLICATION_EXPERIENCE_LOAD_FAILED'});}});

  app.patch('/api/apps/:id/experience',auth,async(req,res)=>{try{await authTablesReady;const userId=req.mtpSession.userId;const body=req.body||{};const [owned]=await pool.execute('SELECT 1 FROM user_applications WHERE user_id=? AND application_id=? LIMIT 1',[userId,req.params.id]);if(!owned.length)return res.status(404).json({error:'APPLICATION_NOT_FOUND'});if(body.title!==undefined){const title=String(body.title||'').trim().slice(0,160);if(!title)return res.status(400).json({error:'INVALID_APPLICATION_TITLE'});await pool.execute('UPDATE applications SET title=?,updated_at=CURRENT_TIMESTAMP WHERE id=?',[title,req.params.id]);}if(Object.hasOwn(body,'customIconData')){let iconData=null;if(body.customIconData!==null){iconData=String(body.customIconData);if(!/^data:image\/(png|jpeg|jpg|webp|svg\+xml);base64,[A-Za-z0-9+/=]+$/.test(iconData)||iconData.length>700000)return res.status(400).json({error:'INVALID_ICON_DATA'});}await pool.execute('UPDATE user_applications SET custom_icon_data=?,updated_at=CURRENT_TIMESTAMP WHERE user_id=? AND application_id=?',[iconData,userId,req.params.id]);}res.json({ok:true});}catch{res.status(400).json({error:'APPLICATION_EXPERIENCE_UPDATE_FAILED'});}});
