...
export function getVexaConfig() {
  ...
  if (!url || !clientId || !redirectUri || !clientSecret) throw new Error('VEXA_SSO_NOT_CONFIGURED');
  return { url, clientId, redirectUri, scopes, timeoutMs, clientSecret };
}
...
export function buildAuthorizeUrl(transaction) {
  ...
  const url = new URL('/api/sso/authorize', cfg.url);
...
export async function exchangeAuthorizationCode(code, verifier) {
  ...
  const response = await fetch(new URL('/api/sso/token', cfg.url), {
    method: 'POST',
...
export async function fetchVexaUser(accessToken) {
  const cfg = getVexaConfig();
  const response = await fetch(new URL('/api/sso/userinfo', cfg.url), {
    headers: { Authorization: `Bearer ${accessToken}` },
    signal: AbortSignal.timeout(Math.min(cfg.timeoutMs, 7000))
  });
...
export function encryptSecret(value) {
  const key = crypto.createHash('sha256').update(process.env.MTP_SESSION_ENCRYPTION_KEY || process.env.VEXA_ACCOUNT_CLIENT_SECRET || '').digest();
